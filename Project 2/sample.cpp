...
Animate()
{
	...
	float BladeSpeed = 4;
	BladeAngle = (360.0f * Time) * BladeSpeed;
...
Display()
{
	...
	if (COCKPIT)
	{
		gluLookAt(-0.4f, 1.8f, -4.9f, -0.4f, 1.8f, -10.0f, 0.0f, 1.0f, 0.0f);
	}
	else
	{
		gluLookAt(18.f, 8.f, 4.f, 0.f, 2.f, -2.f, 0.f, 1.f, 0.f);
		// rotate the scene:
	...
	// Draw shape
	glPushMatrix();
	glScalef(10.0f, 10.0f, 10.0f);			// Scale to 10
	glTranslatef(0.0f, 0.0f, -1.5f);
	glCallList(SymbolList);
	glPopMatrix();

	// Draw the main rotor
	glPushMatrix();
	glTranslatef(0.0f, 2.9f, -2.0f);
	glRotatef(BladeAngle, 0., 1., 0.); // Used for animating the blade. 
	glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
	glScalef(5.0f, 5.0f, 5.0f);			// Scale to 5
	...
	glRotatef(fmod(BladeAngle * 2.0f, 360.0f), 1., 0., 0.); // Used for animating the blade at 2X speed. 
	glRotatef(90.f, 0.0f, 1.0f, 0.0f);
	glScalef(3.0f, 3.0f, 3.0f);			// Scale to 3
	glCallList(BladeList);
	glPopMatrix();
...
void CreateHeli()
{
	...
	for (i = 0, tp = Helitris; i < Helintris; i++, tp++)
	{
		...
		if (n[1] > 1.)
			n[1] = 1.;
		glColor3f(1.0f * n[1], 0.34f * n[1], 0.00f);
...
InitLists()
{
	...
	// Create the moutain thing
	SymbolList = glGenLists(1);
	glNewList(SymbolList, GL_COMPILE);
	CreateShape();
	glEndList();
